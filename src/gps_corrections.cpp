class Gps_Corrections
{
  ...
  public:
    ...
    *-----------------------------------------------------------------------------*/
    static double tropmapf(gtime_t time, const double pos[], const double azel[1], double *mapfw)
    {
    ...
    *-----------------------------------------------------------------------------*/
    static double tropmodel(gtime_t time, double pos[3], double azel[1], double humi)
    {
    ...
    *-----------------------------------------------------------------------------*/
    static double ionmodel(gtime_t t, const double ion[7], double pos[3], double azel[1])
    {
  ...
  // applying corrections for ionosphere and troposphere to psedo range
  // clock correction is already applied by the ublox receiver
  static void apply_corr(std::vector<sat_pos> *sp_arr, double base_station_position[3])
  {
    double ion_delay;
    double tropo_delay;
    ...
    // missing because not available in sfrbx and rawx msg
    double azel[1] = {0};
    double humi = 0;
    ...
    {
      // requires ion model params and azel as extern params
      ion_delay = ionmodel(time_now, (*sp_arr)[i].ion_params, pos, azel);

      // requires humidity and azel as exteren params
      tropo_delay = tropmodel(time_now, pos, azel, humi);
    }
  }
};
