...
class M8T
{
  ...
  static void print_sat_pos_array(std::vector<sat_pos> *satellites_array, int show_only_valid_sats)
  {
    printf("------------------%d------------------- \n", satellites_array->size());
    if(satellites_array->size()>0){
      for (int i=0; i<satellites_array->size();i++)
      {
        if(show_only_valid_sats && check_gps_data_validity(&(*satellites_array)[i]))
        {
          ...
          std::cout << "pseudo_range_observed: " << (*satellites_array)[i].pseudo_range_calc << std::endl;
          std::cout << "pseudo_range_calc: " << (*satellites_array)[i].pseudo_range_observed << std::endl;
